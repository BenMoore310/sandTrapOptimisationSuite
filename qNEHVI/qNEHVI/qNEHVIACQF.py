...
class qLogNoisyExpectedHypervolumeImprovementClassifier(
    NoisyExpectedHypervolumeMixin,
    ...
    def forward(self, X: Tensor) -> Tensor:
        ...
        # check what magnitude of value nehvi returns
        # print("NEHVI value:", nehvi.exp().item())
        # print("plain NEHVI value:", nehvi)

        # feed X through feasibility classifier

        # print("X shape:", X.shape)

        feaseValues = Classifier.GPEval(
            self.feaseModel, self.feaseLikelihood, X.squeeze(1)
        )
        feaseValues = feaseValues.clamp(min=1e-6)
        feaseValues = feaseValues.log()
        # print("log feasibility value:", feaseValues)

        # print(nehvi.shape, feaseValues.mean.T.shape)

        # multiply nehvi by probability of feasibility

        # alpha = 1.5

        nehvi = nehvi + feaseValues

        # print("Modified NEHVI value:", nehvi)

        if self.incremental_nehvi:
            # print('incremental is true')
            return nehvi
