...
class qLogNoisyExpectedImprovement(
    ...
    def __init__(
        ...
        self.feaseModel = feaseModel
        self.feaseLikelihood = feaseLikelihood
        if incremental:
            self.X_pending = None  # required to initialize attribute for optimize_acqf
        self.currentX = None  # trying to pass the X coords to the logEI calculation
        self._init_baseline(
            model=model,
    ...
    def _sample_forward(self, obj: Tensor) -> Tensor:
        ...
        Returns:
            ...
            improvement values.
        """
        # print('obj:', obj, obj.shape)
        # print('in sampleForwards:', self.currentX)
        li = _log_improvement(
            Y=obj,
        ...
        # print('logImprovement:', li, li.shape)

        feaseValues = torch.reshape(
            Classifier.GPEval(
                self.feaseModel, self.feaseLikelihood, self.currentX.squeeze(1)
            ),
            (int(len(obj[1])), 1),
        )

        feaseValues = feaseValues.clamp(min=1e-6)
        feaseValues = feaseValues.log()
        # print('feaseValues:', feaseValues)
    ...
    def set_X_pending(self, X_pending: Tensor | None = None) -> None:
        ...
        if X_pending is None:
            if not hasattr(self, "_full_X_baseline") or (
                ...
                X_pending = None
        # self.X_pendingForClass = X_pending
        # print('X_pendingForClass:', self.X_pendingForClass)
        self._init_baseline(
            model=self.model,
    ...
    def _get_samples_and_objectives(self, X: Tensor) -> tuple[Tensor, Tensor]:
        ...
        # print('X_full:', X_full, X_full.shape)
        ...
        # the sample values are drawn from the posterior, created from X (?) so I need to modify what the model
        # posterior is constructed from...?
        ...
        # print('initial obj:', obj, obj.shape)
        # print('initial samples:', samples)

        # print(obj.shape, samples.shape)

        # squeezed = samples.squeeze(2)
        # print(squeezed.shape)

        # feaseValues = torch.reshape(Classifier.GPEval(self.feaseModel, self.feaseLikelihood, X.squeeze(1)),
        #                             (int(len(obj[1])),1))

        # feaseValues = (feaseValues > 0.75).float()

        # print('fease values:', feaseValues)
        # obj = obj * feaseValues
        # print('altered obj:', obj, obj.shape)
        return samples, obj
