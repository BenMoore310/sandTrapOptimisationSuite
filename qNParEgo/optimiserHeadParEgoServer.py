...
from qNParEgo import qNParEgoClassifier
import math
...
logging.basicConfig(
    filename="parEgo.log", encoding="utf-8", filemode="w", level=logging.DEBUG
)
...
#     logger.info(f"Running on server: {server}")
#     subprocess.run(
#         ["bash", "allRun", server],
...
def prepareRunDir(sample, numBasis):
    ...
    process = subprocess.Popen(
        ["bash", "allrunInit"],
        cwd="/home/bm424/Projects/sandTrapShapeOptBenchmarking/qNParEgo/parEgoCaseDir",
...
def simulateDesign(objective, sample, numBasis):
    ...
    cwdPath = "/home/bm424/Projects/sandTrapShapeOptBenchmarking/qNParEgo"

    serverName = "cfd6"

    objective_strings = ["0.025", "0.075", "1.125"]

    flowRate = objective_strings[objective]

    # subprocess.run(["bash", "runServerJob2", serverName, flowRate], cwd=cwdPath, check=True)
...
def main(numBasis, numObj, initialSamples, seed, classifierMC=False):
    ...
    # logger.info(f"validInitPop.shape)
    ...
    qNParEgo = qNParEgoClassifier.qNParEgo()
