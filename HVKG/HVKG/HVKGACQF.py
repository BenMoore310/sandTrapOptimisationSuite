...
class HVKGClassifier(qHypervolumeKnowledgeGradient):
    def __init__(self, feaseModel, feaseLikelihood, *args, **kwargs):
    ...
    def forward(self, X):
        ...
        out = super().forward(X)
        # print("initial out:", out, out.shape)
        # print("x_actual", X_actual, X_actual.shape)
        # print(X.shape)
        # print(X.squeeze(0).shape)
        # print(X)
        feaseValues = Classifier.GPEval(
            self.feaseModel, self.feaseLikelihood, X_actual.squeeze(0)
        )
        # out = out * feaseValues
        # print("feaseValue:", feaseValues)

        out = out - ((1 - feaseValues) * abs(out))

        # if out < 0:
        #     out = out * (1+feaseValues)
        # else:
        #     out = out * feaseValues

        # print("final out:", out, out.shape)
...
# class qHypervolumeKnowledgeGradientClassifier(
...
# class qMultiFidelityHypervolumeKnowledgeGradient(qHypervolumeKnowledgeGradientClassifier):
